...
def equilbrium(x):
    ...
    for i in range(1,len(x)):
        if x[i] == x[i-1]: #  checking the previcomputer simulationous and the current positions are the same
            return i-1 #  returning the positioncomputer simulation once the previous and current items are the same
